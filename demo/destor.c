 ...
 * server.c
 *
 *  /Users/apple/Documents/destor/configureCreated on: May 24, 2012
 *      Author: fumin
 */
...
void check_simulation_level(int last_level, int current_level) {
	if ((last_level <= SIMULATION_RESTORE && current_level >= SIMULATION_APPEND)
		...
		fprintf(stderr, "FATAL ERROR: Conflicting simualtion level!last =%d, curr=%d\n", last_level, current_level);
		exit(1);
	}
}
...
void destor_start() {
	...
	/* Init */
	destor.working_directory = sdsnew("./");
...
void destor_stat() {
	...
	if(destor.simulation_level == SIMULATION_FSL_DATA)
		printf("simulation is %s\n", "FSL_DATA");
	else if (destor.simulation_level == SIMULATION_NO)
