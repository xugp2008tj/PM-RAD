...
void load_config_from_string(sds config) {
	...
	for (i = 0; i < totlines; i++) {
		...
		if (strcasecmp(argv[0], "working-directory") == 0 && argc == 2) {
			...
			if (strcasecmp(argv[1], "fsl") == 0) {
				destor.simulation_level = SIMULATION_FSL_DATA;
			...
			if (argc > 3) {
                ...
                } else if (strcasecmp(argv[3], "learn") == 0) {
                    assert(destor.index_category[0] == INDEX_CATEGORY_NEAR_EXACT
                           && destor.index_category[1] == INDEX_CATEGORY_LOGICAL_LOCALITY);
                    destor.index_specific = INDEX_SPECIFIC_LEARN;
                } else {
        ...
        } else if (strcasecmp(argv[0], "cst-index-value-length") == 0
                   && argc == 2) {
            destor.cst_value_length = atoi(argv[1]);
        } else if (strcasecmp(argv[0], "update-segment-policy") == 0
                   && argc == 2) {
            if (strcasecmp(argv[1], "fifo") == 0)
                destor.update_segment_policy = FIFO_SEGMENT_UPDATE;
            else if (strcasecmp(argv[1], "min") == 0)
                destor.update_segment_policy = MIN_SEGMENT_UPDATE;
            else {
                err = "Invalid segment update algorithm";
                goto loaderr;
            }

        } else if (strcasecmp(argv[0], "segment-select-method")
                 == 0 && argc >= 2) {
            if (strcasecmp(argv[1], "greedy") == 0)
                destor.select_segment_policy = GREEDY_SEGMENT_SELECT;
            else if (strcasecmp(argv[1], "random") == 0)
                destor.select_segment_policy = RANDOM_SEGMENT_SELECT;
            else if (strcasecmp(argv[1], "recent") == 0)
                destor.select_segment_policy = RECENT_SEGMENT_SELECT;
            else {
                err = "Invalid segment select algorithm";
                goto loaderr;
            }
            if (argc > 2) {
                assert(destor.select_segment_policy == GREEDY_SEGMENT_SELECT);
                destor.epsilon = atof(argv[2]);
            }
        }
