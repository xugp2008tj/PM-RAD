...
static void* filter_thread(void *arg) {
    ...
    while (1) {
        ...
        for (; iter != end; iter = g_sequence_iter_next(iter)) {
            ...
            DEBUG("Filter phase: %dth chunk in %s container %lld", chunk_num,
            ...
            if (!CHECK_CHUNK(c, CHUNK_DUPLICATE) || (!CHECK_CHUNK(c, CHUNK_REWRITE_DENIED)
                ...
                if(add_chunk_to_container(storage_buffer.container_buffer, c)){
                	...
                	if (!CHECK_CHUNK(c, CHUNK_DUPLICATE)) {
                    	...
                    	DEBUG("Filter phase: %dth chunk is recently unique, size %d", chunk_num,
                	...
                	DEBUG("Filter phase: container %lld already has this chunk", c->id);
                ...
                if(CHECK_CHUNK(c, CHUNK_REWRITE_DENIED)){
                    DEBUG("Filter phase: %lldth fragmented chunk is denied", chunk_num);
                }else if (CHECK_CHUNK(c, CHUNK_OUT_OF_ORDER)) {
                    DEBUG("Filter phase: %lldth chunk in out-of-order container %lld is already cached",
                            chunk_num, c->id);
                }
            }
