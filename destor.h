...
#define GREEDY_SEGMENT_SELECT 0
#define RECENT_SEGMENT_SELECT 1
#define RANDOM_SEGMENT_SELECT 2


#define MIN_SEGMENT_UPDATE 0
#define FIFO_SEGMENT_UPDATE 1
...
struct destor {
    ...
    /*-------------------------------*/
    int select_segment_policy;
    //GREEDY_SEGMENT_SELECT, RECENT_SEGMENT_SELECT, RANDOM_SEGMENT_SELECT
    int update_segment_policy;
    //FIFO_SEGMENT_UPDATE, MIN_SEGMENT_UPDATE
    int cst_value_length;
    double epsilon;
    /*-------------------------------*/
...
//-------------------------------
typedef struct cst_entry_segment {
    /*the first segment should be prefetched*/
    segmentid id;
    /*current score of this entry*/
    int score;
    /*the number of chunks to prefect: default value is set 
     by configuration parameter "fingerprint-index-segment-prefetching"
     which is to set destor.index_segment_prefech.
     */
    int prefetch_num;
    /*the curent number of its subsequent segments including itself in cache*/
    int num;
}cst_entry_segment;
...
struct segment {
	segmentid id;
	/* The actual chunk number because there are signal chunks. */
	int32_t chunk_num;
    /*used for retore*/
