...
void init_index() {
    ...
    if(destor.index_specific != INDEX_SPECIFIC_NO){
        ...
        switch(destor.index_specific){
            ...
            case INDEX_SPECIFIC_LEARN:{
                assert(destor.index_category[0] == INDEX_CATEGORY_NEAR_EXACT);
                assert(destor.index_category[1] == INDEX_CATEGORY_LOGICAL_LOCALITY);
...
void close_index() {
    close_fingerprint_cache();
    if(destor.index_specific == INDEX_SPECIFIC_LEARN)
        close_cst();
    else
        close_kvstore();
    NOTICE("Close index module successfully");
}
        ...
        if (!CHECK_CHUNK(c, CHUNK_DUPLICATE)) {
            ...
            if(ids){
                ...
                if(id != TEMPORARY_ID){
                    ...
                    NOTICE("Dedup phase: A key collision occurs---base");
                }
            }else{
                index_overhead.lookup_requests_for_unique++;
                NOTICE("Dedup phase: non-existing fingerprint---base");
            }
        }
...
extern void index_lookup_learning(struct segment *s);
...
int index_lookup(struct segment* s) {
    ...
    if (destor.index_specific == INDEX_SPECIFIC_LEARN) {
        ...
        //NOTICE("there are %d chunks in the segment (id=%lld) to sample features", s->chunk_num, s->id);
        index_lookup_learning(s);
    }
...
void index_update(GHashTable *features, int64_t id){
    //NOTICE("Filter phase: update %d features with segment id %lld", g_hash_table_size(features), id);
