...
//m: the required number of bits m
//p: error, a desired false positive probability p
//n: est_key_count, the given number of inserted elements
//k: filter->hashes_count, the number of hash functions
bf_t* bf_create(
	...
	bf_index_t (*hash_func)(char*, uint))
{
    ...
    //
	filter->bits_count = (bf_index_t)ceil(- (double)est_key_count * log(error) / BF_LOG2_2);//log以e为底
    ...
    // the value of k (the number of hash functions) that minimizes the false positive probability
...
void bf_add(bf_t* filter, const char* key, uint key_len)
{
	...
	for (hash_index = 0; hash_index < filter->hashes_count; hash_index++) {
        bf_index_t t = bf_get_index(filter, key, key_len, hash_index);
		uint changed = bf_bitmap_set(filter->bitmap, t);
